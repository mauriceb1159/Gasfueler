...
export function BookingForm({
  ...
  const storeFirstCartSummary = (
    <StoreFirstCartSummary
      ...
      onUpdateItemQuantity={updateStoreItemQuantity}
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-br from-orange-100 via-amber-50 to-white text-[10px] font-semibold uppercase tracking-[0.16em] text-orange-700">
                          {item.storeItem.imageUrl ? (
                            <img
                              src={item.storeItem.imageUrl}
...
function StoreFirstCartSummary({
  ...
  catalogCount: number;
  onUpdateItemQuantity: (stationStoreItemId: number, quantity: number) => void;
  bagItems: {
    id: number;
    name: string;
    quantity: number;
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-4">
          ...
          <div className="mt-3 space-y-2">
            {bagItems.map((item) => (
              <div
                key={item.id}
                className="flex items-center justify-between gap-4 rounded-2xl border border-slate-100 bg-slate-50/70 px-3 py-2 text-sm"
              >
                <div className="min-w-0 flex-1">
                  <p className="truncate text-slate-600">{item.name}</p>
                  <div className="mt-2 inline-flex items-center rounded-full border border-slate-200 bg-white p-1">
                    <button
                      type="button"
                      onClick={() =>
                        onUpdateItemQuantity(item.id, item.quantity - 1)
                      }
                      className="flex h-7 w-7 items-center justify-center rounded-full text-base text-slate-600 transition hover:bg-slate-100 hover:text-slate-950"
